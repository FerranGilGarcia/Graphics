...
uniform vec4 u_backgroundColor;           // Color de fondo (B)
...
uniform float u_absorptionCoefficient;    // Coeficiente de absorción (μa)
uniform vec3 u_emissionColor;             // Color de emisión (Le)
uniform float u_emissionIntensity;        // Intensidad de emisión
...
// Función de ruido fractal para generar densidad
float hash1(float n) {
...
void main() {
    ...
    vec3 accumulatedRadiance = vec3(0.0); // Acumulador para la radiancia emitida
    ...
    for (int i = 0; i < u_numSteps; i++) {
        ...
        // Calcular la transmitancia parcial en este paso
        float transmittance = exp(-opticalThickness);

        // Calcular la radiancia emitida y acumularla
        vec3 emittedRadiance = u_emissionColor * u_emissionIntensity * density * transmittance;
        accumulatedRadiance += emittedRadiance * stepSize;

        // Avanzar a la siguiente posición de la muestra
        samplePosition += rayDirection * stepSize;
    }

    // Calcular la transmitancia total usando la ley de Beer-Lambert
    float finalTransmittance = exp(-opticalThickness);

    // Calcular el color final aplicando la transmitancia al color de fondo y sumando la radiancia emitida acumulada
    FragColor = vec4(u_backgroundColor.rgb * finalTransmittance + accumulatedRadiance, 1.0);
}
